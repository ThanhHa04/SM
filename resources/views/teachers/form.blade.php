...
<form id="form" class="text-start" method="POST"
    action="{{isset($rec) ? route('teachers.update', ['id' => $rec->teacherProfile->id]) : route('teachers.create')}}">
    {{ csrf_field() }}
    ...
    <div class="input-group input-group-outline opacity-7">
        <input type="text" name="teacher_id" class="form-control opacity-10" required
            value="{{ old('teacher_id', $rec->teacherProfile->teacher_id ?? ($teacher_id ?? '')) }}" readonly>    
    </div>
    
    <label class="form-label mt-3">Họ và tên *</label>
    <div class="input-group input-group-outline">
        <input type="text" name="name" class="form-control" required value="{{$rec->teacherProfile->name ?? old('name') ?? ''}}">
    </div>
   ...
   <div class="input-group input-group-outline">
      <input type="tel" name="phone_number" class="form-control" placeholder="Nhập số"
        ...
        value="{{ old('phone_number', $rec->teacherProfile->phone_number ?? '') }}">
   </div>
    ...
    <div class="input-group input-group-outline">
        <input type="date" name="dob" class="form-control" required value="{{date('Y-m-d', strtotime($rec->teacherProfile->dob ?? old('dob') ?? ''))}}">
    </div>
    ...
    <div class="input-group input-group-outline">
        <select name="gender" class="form-control" required>
            ...
            <option value="Nam" {{ old('gender', $rec->teacherProfile->gender ?? '') == 'Nam' ? 'selected' : '' }}>Nam</option>
            <option value="Nữ" {{ old('gender', $rec->teacherProfile->gender ?? '') == 'Nữ' ? 'selected' : '' }}>Nữ</option>
        </select>
    </div>
